...
- Mark it as a PARTIAL MATCH if there is some clearly related experience but less extensive or clearly less senior than the requirement wording.
  - Example: requirement says "Production experience with Kubernetes orchestration and deployment" and candidate only mentions "Kubernetes training" or a small personal project -> PARTIAL MATCH.
...
- Else:
  ...
  - IMPORTANT: If there is at least one FULL MATCH or PARTIAL MATCH, the score MUST be > 0.
- Output:
  ...
  - must_have_skills.analysis: a concise explanation of which requirements are fully met, which are partially met (and why), and which are missing.
...
- Output:
  ...
  - nice_to_have_skills.analysis: concise explanation grounded in evidence.
...
- Read required_years from job_data["requirements"]["minimum_years_experience"].
  - If missing or null, treat as 0.
- Derive candidate_years from candidate_data["work"]:
  - Use explicit startDate and endDate values.
  - If endDate is null or missing, treat it as current date (or present) and compute duration up to now.
  - Sum durations across roles to get total professional experience in years (you may approximate to one decimal).
...
- Output:
  - minimum_years_experience.candidate_years: numeric estimate
  - minimum_years_experience.required_years: value from job_data["requirements"]["minimum_years_experience"]
  - minimum_years_experience.analysis: explain how you derived candidate_years and how it compares to required_years.
...
- Search for evidence across ALL parts of candidate_data:
  - basics, skills, work (position, summary, highlights), projects, education, certifications.
- Strengths:
  - List 2-5 concise strengths grounded in job requirements and candidate evidence
    (e.g., tech stack matches, years of experience, concrete achievements, relevant domains).
- Weaknesses:
  - List 2-5 concise weaknesses / risk factors grounded in job requirements
    (e.g., missing Kubernetes, no microservices experience, short on years, not yet senior/lead).
...
- Do not leave analysis or summary blank; provide at least one sentence each.

Return the result using this structured schema:
- overall_score (number)
- must_have_skills: {{ "score": number, "analysis": string, "matched_skills": [strings from job_data.requirements.must_have_skills], "missing_skills": [strings from job_data.requirements.must_have_skills] }}
- nice_to_have_skills: same structure using job_data.requirements.nice_to_have_skills
- minimum_years_experience: {{ "score": number, "analysis": string, "candidate_years": number, "required_years": number }}
- required_education: {{ "score": number, "analysis": string, "candidate_education": string or null, "required_education": string or null }}
